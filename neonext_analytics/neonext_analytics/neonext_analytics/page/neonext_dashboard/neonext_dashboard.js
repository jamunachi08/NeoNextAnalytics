...
function render_all($body, date_range) {
	$body.html(`
		...
		<div class="neonext-card neonext-invoices-card">
...
function render_kpis($target, date_range) {
	$target.html(`<div class="neonext-loading">${__("Loading…")}</div>`);
	frappe.call({
		...
		error() {
			$target.html(`<div class="neonext-error">${__("Could not load summary — check your permissions.")}</div>`);
		},
	});
}
...
function render_recent_invoices() {
	...
	$t.html(`<div class="neonext-loading">${__("Loading…")}</div>`);
	frappe.call({
		...
		callback(r) {
			...
			if (!rows.length) {
				$t.html(`<div class="neonext-empty">${__("No invoices found.")}</div>`);
				return;
			}
			let html = `<table class="table table-bordered neonext-table">
				<thead><tr>
					<th>${__("Invoice")}</th>
					<th>${__("Customer")}</th>
					<th>${__("Date")}</th>
		...
		error() {
			$t.html(`<div class="neonext-error">${__("Could not load invoices.")}</div>`);
		},
	});
}
